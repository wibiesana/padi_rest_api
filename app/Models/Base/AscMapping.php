...
class AscMapping extends ActiveRecord
{
    protected string $table = 'asc_mapping';
    protected string|array $primaryKey = 'id';
    
    protected array $fillable = [
        'entity_type', 'asc_id', 'asc_name', 'asc_short', 'local_id', 'local_table', 'is_active', 'last_sync'
    ];
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'asc_mapping.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'asc_mapping.updated_by = users_updated_by.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'asc_mapping.entity_type', $keyword],
                ['LIKE', 'asc_mapping.asc_name', $keyword]
            ]);
    ...
    /**
     * Search asc_mapping (simple limit)
     */
