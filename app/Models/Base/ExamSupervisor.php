...
class ExamSupervisor extends ActiveRecord
{
    protected string $table = 'exam_supervisors';
    protected string|array $primaryKey = 'id';
    
    protected array $fillable = [
        'exam_id', 'teacher_id', 'classroom_id', 'description'
    ];
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('classroom AS classroom', 'exam_supervisors.classroom_id = classroom.id')
            ->leftJoin('exam AS exam', 'exam_supervisors.exam_id = exam.id')
            ->leftJoin('teacher AS teacher', 'exam_supervisors.teacher_id = teacher.id')
            ->where(['OR',
                ['LIKE', 'classroom.name', $keyword],
                ['LIKE', 'exam.name', $keyword],
                ['LIKE', 'teacher.name', $keyword],
                ['LIKE', 'exam_supervisors.description', $keyword]
            ]);
    ...
    /**
     * Search exam_supervisors (simple limit)
     */
