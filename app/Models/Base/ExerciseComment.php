...
class ExerciseComment extends ActiveRecord
{
    protected string $table = 'exercise_comment';
    protected string|array $primaryKey = 'id';
    
    protected array $fillable = [
        'comment', 'exercise_id', 'like', 'dislike', 'rating'
    ];
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'exercise_comment.created_by = users.id')
            ->leftJoin('exercise AS exercise', 'exercise_comment.exercise_id = exercise.id')
            ->leftJoin('users AS users_updated_by', 'exercise_comment.updated_by = users_updated_by.id')
            ->where(['OR',
    ...
    /**
     * Search exercise_comment (simple limit)
     */
