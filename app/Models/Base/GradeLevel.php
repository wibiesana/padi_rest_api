...
{
    protected string $table = 'grade_level';
    protected string|array $primaryKey = 'id';
    
    protected array $fillable = [
        'name', 'full_name', 'level_type', 'sequence', 'description', 'is_active'
    ];
    ...
    {
        return $this->hasMany(\App\Models\Classroom::class, 'grade_level_id');
    }
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'grade_level.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'grade_level.updated_by = users_updated_by.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'grade_level.name', $keyword],
                ['LIKE', 'grade_level.full_name', $keyword],
                ['LIKE', 'grade_level.level_type', $keyword],
                ['LIKE', 'grade_level.description', $keyword]
            ]);
    ...
    /**
     * Search grade_level (simple limit)
     */
