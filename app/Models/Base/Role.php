...
class Role extends ActiveRecord
{
    ...
    {
        return $this->hasOne(\App\Models\UserRole::class, 'role_id');
    }
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'role.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'role.updated_by = users_updated_by.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'role.name', $keyword],
                ['LIKE', 'role.description', $keyword]
            ]);
