...
class StaffUpload extends ActiveRecord
{
    protected string $table = 'staff_upload';
    ...
    {
        return $this->hasMany(\App\Models\StaffUploadResult::class, 'staff_upload_id');
    }
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'staff_upload.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'staff_upload.updated_by = users_updated_by.id')
            ->leftJoin('semester AS semester', 'staff_upload.semester_id = semester.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'semester.name', $keyword],
                ['LIKE', 'staff_upload.name', $keyword],
                ['LIKE', 'staff_upload.description', $keyword],
                ['LIKE', 'staff_upload.status', $keyword]
            ]);
    ...
    /**
     * Search staff_upload (simple limit)
     */
