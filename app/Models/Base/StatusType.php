...
{
    protected string $table = 'status_type';
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'status_type.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'status_type.updated_by = users_updated_by.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'status_type.name', $keyword],
                ['LIKE', 'status_type.description', $keyword],
                ['LIKE', 'status_type.status', $keyword]
            ]);
    ...
    /**
     * Search status_type (simple limit)
     */
