...
class StudentUpload extends ActiveRecord
{
    protected string $table = 'student_upload';
    ...
    {
        return $this->hasMany(\App\Models\StudentUploadResult::class, 'student_upload_id');
    }
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'student_upload.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'student_upload.updated_by = users_updated_by.id')
            ->leftJoin('semester AS semester', 'student_upload.semester_id = semester.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'semester.name', $keyword],
                ['LIKE', 'student_upload.name', $keyword],
                ['LIKE', 'student_upload.description', $keyword],
                ['LIKE', 'student_upload.status', $keyword]
            ]);
    ...
    /**
     * Search student_upload (simple limit)
     */
