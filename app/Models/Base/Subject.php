...
class Subject extends ActiveRecord
{
    ...
    protected array $fillable = [
        'name', 'short_name', 'asc_id', 'asc_partner_id', 'status'
    ];
    ...
    {
        return $this->hasMany(\App\Models\Article::class, 'subject_id');
    }
    ...
    {
        return $this->hasMany(\App\Models\Exam::class, 'subject_id');
    }

    public function teachingschedules()
    {
        return $this->hasMany(\App\Models\TeachingSchedule::class, 'subject_id');
    }
    ...
    public function searchPaginate(string $keyword, int $page = 1, int $perPage = 25, ?string $orderBy = null): array
    {
        ...
        $query = Query::find()
            ->select("{$this->table}.*")
            ->from($this->table)
            ->leftJoin('users AS users', 'subject.created_by = users.id')
            ->leftJoin('users AS users_updated_by', 'subject.updated_by = users_updated_by.id')
            ->where(['OR',
                ['LIKE', 'users.username', $keyword],
                ['LIKE', 'users_updated_by.username', $keyword],
                ['LIKE', 'subject.name', $keyword],
                ['LIKE', 'subject.short_name', $keyword],
                ['LIKE', 'subject.status', $keyword]
            ]);
